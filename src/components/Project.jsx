import { PiShareNetworkBold } from "react-icons/pi";
import { FaGithub } from "react-icons/fa";

function Project() {

    return (
      <>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://currency-converter-khaki-psi.vercel.app/">
                    <img src="src/assets/Currency Converter.png" />
                </a> 
            </div>
            <p>Currency Converter</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Currency-Converter">
                <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://currency-converter-khaki-psi.vercel.app/">
               <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://tic-tac-toe-game-gamma-six.vercel.app/">
                    <img src="src/assets/Tic-Tac-Toe Game.png" />
                </a> 
            </div>
            <p>Tic-Tac-Toe Game</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Tic-Tac-Toe-Game">
                <FaGithub className="icon"/> GitHub Link</a>
                <a target="_blank" href="https://tic-tac-toe-game-gamma-six.vercel.app/">
                 <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://durga-jaiswal.github.io/Shopping-List-App/">
                    <img src="src/assets/Shopping App.png" />
                </a> 
            </div>
            <p>Shopping Cart App</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Shopping-List-App-using-Firebase">
                <FaGithub className="icon"/> GitHub Link</a>
                <a target="_blank" href="https://durga-jaiswal.github.io/Shopping-List-App/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://password-generator-eight-ecru.vercel.app/">
                    <img src="src/assets/Password Generator.png" />
                </a> 
            </div>
            <p>Password Generator</p>
            <div className="link">
            <a target="_blank" href="https://github.com/Durga-Jaiswal/Password-Generator-">
                <FaGithub className="icon"/> GitHub Link</a>
                <a target="_blank" href="https://password-generator-eight-ecru.vercel.app/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://lorem-ipsum-generator-omega.vercel.app/">
                    <img src="src/assets/Lorem-Ipsum Generator.png" />
                </a> 
            </div>
            <p>Lorem-Ipsum Generator</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Lorem-Ipsum-Generator">
                <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://lorem-ipsum-generator-omega.vercel.app/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://faq-accordion-puce-three.vercel.app/">
                    <img src="src/assets/FAQ Accordion.png" />
                </a> 
            </div>
            <p>FAQ Accordion</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/FAQ-Accordion">
                <FaGithub className="icon"/> GitHub Link</a>
                <a target="_blank" href="https://faq-accordion-puce-three.vercel.app/">
                 <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://review-page-dusky.vercel.app/">
                    <img src="src/assets/Review Slider.png" />
                </a> 
            </div>
            <p>Review Slider</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Review-Page"> 
                    <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://review-page-dusky.vercel.app/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://tours-project-delta.vercel.app/">
                    <img src="src/assets/Food Menu.png" />
                </a> 
            </div>
            <p>Tour Site</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Tours-Project">
                    <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://tours-project-delta.vercel.app/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://tab-component-sable.vercel.app/">
                    <img src="src/assets/Tab-Component.png" />
                </a> 
            </div>
            <p>Tab Component</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Tab-Compone">
                <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://tab-component-sable.vercel.app/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://durga-jaiswal.github.io/Advice-Generator-Project/">
                    <img src="src/assets/Advice Generator.png" />
                </a> 
            </div>
            <p>Advice Generator</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Advice-Generator-Project">
                <FaGithub className="icon"/>GitHub Link</a>
               
                <a target="_blank" href="https://durga-jaiswal.github.io/Advice-Generator-Project/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://durga-jaiswal.github.io/Age---Calculator---App/">
                    <img src="src/assets/Age Calculator.png" />
                </a> 
            </div>
            <p>Age Calculator</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Age---Calculator---App">
                <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://durga-jaiswal.github.io/Age---Calculator---App/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://durga-jaiswal.github.io/Preview-Card-Component/">
                    <img src="src/assets/Preview Card.png" />
                </a> 
            </div>
            <p>Preview Card</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Preview-Card-Component">
                <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://durga-jaiswal.github.io/Preview-Card-Component/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://durga-jaiswal.github.io/Rating-Component/" >
                    <img src="src/assets/Rating Card.png" />
                </a> 
            </div>
            <p>Rating Card</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Rating-Component">
                <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://durga-jaiswal.github.io/Rating-Component/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://durga-jaiswal.github.io/Result-Summary-Project/">
                    <img src="src/assets/Result Summary.png" />
                </a> 
            </div>
            <p>Result Summary</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Result-Summary-Project">
                <FaGithub className="icon"/> GitHub Link</a>
                <a target="_blank" href="https://durga-jaiswal.github.io/Result-Summary-Project/">
                <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
        <div className="project-card">
            <div className="img-container" >
                <a target="_blank" href="https://durga-jaiswal.github.io/Product-Preview-Card-Component/" >
                    <img src="src/assets/Product Preview Card.png" />
                </a> 
            </div>
            <p>Product Preview Card</p>
            <div className="link">
                <a target="_blank" href="https://github.com/Durga-Jaiswal/Product-Preview-Card-Component ">
                <FaGithub className="icon"/>GitHub Link</a>
                <a target="_blank" href="https://durga-jaiswal.github.io/Product-Preview-Card-Component/">
               <PiShareNetworkBold className="icon"/>Live Site</a>
            </div>
        </div>
      </>
    )
  }
  
  export default Project;
  